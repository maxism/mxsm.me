...
export function boot(options = {}) {
  ...
  const mainCanvas = /** @type {HTMLCanvasElement | null} */ (document.getElementById("stage"));
  ...
  audio = createAudioSystem(
    ...
    (event) => {
      ...
      if (reg === "abyss") {
        voiceModeTarget = 1;
        voiceModeDecayAt = performance.now() + 4200;
      }
      if (reg === "threshold") {
        voiceModeTarget = 2;
        voiceModeDecayAt = performance.now() + 3600;
      }
      if (reg === "max") {
        voiceModeTarget = 3;
        voiceModeDecayAt = performance.now() + 3200;
      }
    },
    {
      locale,
  ...
  function consoleOracle(sceneName, depth) {
    ...
    const depthNote =
      depth === 3 ? phrases.ORACLE_NOTES.silence : depth === 2 ? phrases.ORACLE_NOTES.pattern : "";
    const sceneColor =
      {
        NORMAL: "#3a7055",
        BREAKING: "#904030",
  ...
  function loop(now) {
    ...
    if (depth === 3 && coldness > 0.3 && audio.isStarted() && rand() > 0.9982) {
      ...
      voiceModeTarget = 3;
      voiceModeDecayAt = now + 4000; // голос Макса
    }
