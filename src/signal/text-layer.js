...
export function createTextLayer(random, onDissolve, options = {}) {
  ...
  function addEntry(phrase, intensity, now) {
    ...
    else if (layoutRoll > 0.7) layout = "mirror-h";
    ...
    if (register === "abyss") {
      ...
      glitchRate = 0.3; // быстро корраптится
      ...
      glitchRate = 0.035;
      targetAlpha = isDeep ? 1.0 : 0.88 + random() * 0.1;
    }
  ...
  return {
    ...
    update(dt, now, depth, mPulse) {
      ...
      for (let i = entries.length - 1; i >= 0; i--) {
        ...
        // ── IN ────────────────────────────────────────────────────
        if (e.phase === "in") {
          const fadeSpeed = e.register === "abyss" ? 0.38 : e.register === "max" ? 1.6 : 0.85;
          ...
          if (e.register === "threshold") {
            ...
            const cursorPos = Math.min(Math.floor(progress * e.chars.length), e.chars.length - 1);
            ...
            for (const ch of e.chars) {
              ...
              if (progress >= ch.resolveAt) {
                ...
                if (random() < 0.3) ch.display = GLITCH_CHARS[(random() * GLITCH_CHARS.length) | 0];
          ...
          if (e.register === "threshold") {
            ...
            for (let j = 0; j < e.chars.length; j++) {
              ...
              if (j >= corruptStart) {
                ch.display =
                  random() > 0.38 ? GLITCH_CHARS[(random() * GLITCH_CHARS.length) | 0] : " ";
            ...
            for (const ch of e.chars) {
              if (ch.ch === " ") continue;
              if (random() < 0.012 + progress * 0.1) ch.display = " ";
            ...
            for (const ch of e.chars) {
              if (ch.ch === " ") continue;
              if (random() < gRate + progress * 0.4) {
                ch.display =
                  random() > 0.45 ? GLITCH_CHARS[(random() * GLITCH_CHARS.length) | 0] : " ";
              } else if (random() < 0.04) {
                ch.display = ch.ch;
              }
          ...
          for (const ch of e.chars) {
            ...
            if (random() < 0.15 + outP * 0.45) {
              ch.display =
                random() > 0.35 ? GLITCH_CHARS[(random() * GLITCH_CHARS.length) | 0] : " ";
  ...
  function drawHorizontal(ctx, e, fontSize, w, h) {
    ...
    const fullWidth =
      e.chars.reduce((acc, ch) => acc + ctx.measureText(ch.ch).width + letterGap, 0) - letterGap; // последний gap не нужен
  ...
  function drawVertical(ctx, e, fontSize, w, h) {
    ...
    const totalH = e.chars.reduce((sum, ch) => sum + (ch.ch === " " ? lineH * 0.45 : lineH), 0);
...
function drawGhostHorizontal(ctx, g, fontSize, letterGap, w, h) {
  ...
  ctx.textAlign = "left";
  const fullWidth =
    g.chars.reduce((a, ch) => a + ctx.measureText(ch.ch).width + letterGap, 0) - letterGap;
  ctx.save();
...
function drawGhostVertical(ctx, g, fontSize, w, h) {
  ...
  ctx.textAlign = "center";
  const lineH = fontSize * 1.28;
  const totalH = g.chars.reduce((s, ch) => s + (ch.ch === " " ? lineH * 0.45 : lineH), 0);
  ctx.save();
