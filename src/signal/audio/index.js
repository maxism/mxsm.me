...
export function createAudioSystem(random, onMeaningEvent, options = {}) {
  ...
  const WORLD_URLS = ["/signal/samples/world-1.ogg", "/signal/samples/world-2.ogg"];
  const DARK_URLS = ["/signal/samples/dark-1.oga", "/signal/samples/dark-2.ogg"];

  const AudioCtx = window.AudioContext || window.webkitAudioContext;
  const ctx = AudioCtx ? new AudioCtx() : null;
  if (!ctx)
    return { ensureStarted() {}, update() {}, isStarted: () => false, onPhraseDissolve: null };
  ...
  function scheduleBeat(now, cold = 0) {
    ...
    if (random() > 0.93) {
      nextBeat = now + (60 / heartBpm) * 2.1;
      return;
    }
    const t = now;
    const o1 = ctx.createOscillator();
    const a1 = ctx.createGain();
    o1.frequency.value = 55 + random() * 10;
    o1.type = "sine";
    ...
    o2.frequency.value = 46 + random() * 8;
    o2.type = "sine";
  ...
  function scheduleDrone(now) {
    ...
    osc.connect(amp);
    amp.connect(baseVoiceGain);
    baseVoiceGain.connect(shaper);
  ...
  const isVowel = locale === "en" ? (ch) => /[aeiouy]/.test(ch) : (ch) => /[аеёиоуыэюя]/.test(ch);
  ...
  function scheduleVowel(ch, when) {
    ...
    oscMain.frequency.exponentialRampToValueAtTime(fund * (0.9 + random() * 0.2), when + dur);
    ...
    oscSub.frequency.exponentialRampToValueAtTime(fund * 0.5 * (0.9 + random() * 0.2), when + dur);
    ...
    bp1.type = "bandpass";
    bp1.frequency.value = f1 + (random() - 0.5) * 45;
    bp1.Q.value = 8;
    bp2.type = "bandpass";
    bp2.frequency.value = f2 + (random() - 0.5) * 75;
    bp2.Q.value = 7;
    bp3.type = "bandpass";
    bp3.frequency.value = f3 + (random() - 0.5) * 110;
    bp3.Q.value = 5;
    ...
    hp.connect(bp2);
    hp.connect(bp3);
    bp1.connect(phraseBus);
    bp2.connect(phraseBus);
    bp3.connect(phraseBus);

    oscMain.start(when);
    oscMain.stop(when + dur + 0.02);
    oscSub.start(when);
    oscSub.stop(when + dur + 0.02);
    return dur * 0.88;
  }
  ...
  function scheduleConsonant(ch, when) {
    ...
    amp.gain.setValueAtTime(0.0001, when);
    amp.gain.exponentialRampToValueAtTime(0.07 + random() * 0.05, when + 0.01);
    amp.gain.exponentialRampToValueAtTime(0.0001, when + dur);
    src.connect(bp);
    bp.connect(amp);
    amp.connect(phraseBus);
    src.start(when);
  ...
  function scheduleTunnelPulse(now) {
    ...
    nextTunnelPulse = now + 2.15 + random() * 0.5;
    const t = now;
    const osc = ctx.createOscillator();
    const amp = ctx.createGain();
    osc.type = "sine";
  ...
  function onPhraseDissolve(phrase) {
    ...
    a2.gain.exponentialRampToValueAtTime(0.0001, t + 0.7);

    const filt = ctx.createBiquadFilter();
    filt.type = "bandpass";
    filt.frequency.value = baseHz * 2.2;
    filt.Q.value = 10;
    o1.connect(filt);
    o2.connect(filt);
    filt.connect(a1);
    filt.connect(a2);
    a1.connect(phraseBus);
    a2.connect(phraseBus);
    o1.start(t);
    o1.stop(t + 0.82);
    o2.start(t + 0.04);
    o2.stop(t + 0.82);
  ...
  return {
    ...
    update({ depth, pointerX, pointerY, idleMs, now, meaningPulse: mPulse, coldness = 0 }) {
      ...
      colorFilter.frequency.setTargetAtTime(
        55 + (1 - pointerY) * 520 * depthBias * coldFiltMult,
        t,
        0.16,
      );
      ...
      feedback.gain.setTargetAtTime((depth === 1 ? 0.56 : 0.35) * (1 - coldness * 0.45), t, 0.45);
      delay.delayTime.setTargetAtTime(0.11 + pointerX * 0.34 + (depth === 1 ? 0.12 : 0), t, 0.2);

      noiseGain.gain.setTargetAtTime(
        breathAmt + (1 - pointerY) * 0.06 + (idleMs > 10000 ? 0.08 : 0),
        t,
        0.22,
      );
      ...
      worldBus.gain.setTargetAtTime(
        Math.min(worldLevel + (idleMs > 10000 ? 0.02 : 0), 0.14),
        t,
        0.4,
      );
      ...
      beatGain.gain.setTargetAtTime(
        depth === 1 ? 0.07 + (idleMs > 8000 ? 0.04 : 0) : 0.04 + coldness * 0.04,
        t,
        0.6,
      );
